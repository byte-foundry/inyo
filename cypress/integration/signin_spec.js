describe('Sign in', function() {
	it('should require an email and a password', function() {
	...
	it('should require a valid email', function() {
		...
		cy.get('input[name=email]')
			.type('a')
			.should('have.value', 'a').blur();

		cy.contains('email n\'est pas valide');
	});

	it('should require a valid account', function() {
		cy.visit('http://localhost:3000/auth/sign-in');

		cy.get('input[name=email]')
			.type('doesnot@exist.account')
			.should('have.value', 'doesnot@exist.account').blur();
	...
	it('should require a valid passord', function() {
		cy.visit('http://localhost:3000/auth/sign-in');
		...
		cy.get('input[name=password]')
			.type('testtest1')
			.should('have.value', 'testtest1').blur();
